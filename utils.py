...
def accuracy(y: np.array, y_pred: np.array):
    ...
    true_pos_and_neg = np.sum(y == y_pred)
    accuracy_val = true_pos_and_neg/len(y)
...
def l2_dist(x1: np.array, x2: np.array):
    ...
    for i, x1_row in enumerate(x1):
        for j, x2_row in enumerate(x2):
            euclid_dist = np.sqrt(np.sum(np.power(x1_row-x2_row, 2)))
            dists[i, j] = euclid_dist
...
def load_data_set(clf_type: str):
    ...
    """
    assert clf_type in (
        'ID3', 'KNN'), 'The parameter clf_type must be ID3 or KNN'
    ...
    attributes_names = list(pd.read_csv(
        train_file_path, delimiter=",", dtype=str, nrows=1).keys())
