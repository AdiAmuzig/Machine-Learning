...
def basic_experiment(x_train, y_train, x_test, y_test, formatted_print=False):
    ...
    id3 = ID3(attributes_names, 0)
    id3.fit(x_train, y_train.tolist())
    id3_y_test = id3.predict(x_test)
    for i in range(len(id3_y_test)):
        if id3_y_test[i] == y_test[i]:
            true_pred += 1
        else:
            false_pred += 1

    acc = true_pred/(false_pred + true_pred)
...
def cross_validation_experiment(plot_graph=True):
    ...
    if best_m is not None and plot_graph:
        util_plot_graph(x=m_choices, y=accuracies_mean,
                        x_label='M', y_label='Validation Accuracy %')
    ...
    # ====== YOUR CODE: ======
    assert len(
        m_choices) >= 5, 'fill the m_choices list with  at least 5 different values for M.'
...
if __name__ == '__main__':
    ...
    data_split = get_dataset_split(
        train_dataset, test_dataset, target_attribute)
