...
class ID3:
    ...
    def info_gain(self, left, left_labels, right, right_labels, current_uncertainty):
        ...
        left_child_E = self.entropy(left, left_labels)
        right_child_E = self.entropy(right, right_labels)

        left_num_of_samples = len(left)
        right_num_of_samples = len(right)
        num_of_samples = left_num_of_samples + right_num_of_samples

        info_gain_value = current_uncertainty - \
            ((left_num_of_samples/num_of_samples) * left_child_E +
             (right_num_of_samples/num_of_samples) * right_child_E)
    ...
    def partition(self, rows, labels, question: Question, current_uncertainty):
        ...
        added_to_true = False
        added_to_false = False

        for i, row in enumerate(rows):
            if question.match(row) == True:
                true_labels.append(labels[i])
                if added_to_true == False:
                    true_rows = [row]
                    added_to_true = True
                else:
                    true_rows = np.vstack((true_rows, row))

            elif question.match(row) == False:
                false_labels.append(labels[i])
                if added_to_false == False:
                    false_rows = [row]
                    added_to_false = True
                else:
                    false_rows = np.vstack((false_rows, row))

        gain = self.info_gain(false_rows, false_labels,
                              true_rows, true_labels,
                              current_uncertainty)
    ...
    def find_best_split(self, rows, labels):
        ...
        best_gain = 0.0
        choosen_feature_num = 0

        num_of_examples = rows.shape[0]
        num_of_featurs = rows.shape[1]

        for i in range(num_of_featurs):
            column = rows[:, i]
            sorted_column = np.sort(column)

            for j in range(num_of_examples - 1):
                question_value = (sorted_column[j] + sorted_column[j+1])/2
                question = Question(column, i, question_value)
                gain, true_rows, true_labels, false_rows, false_labels = self.partition(
                    rows, labels, question, current_uncertainty)

                if gain > best_gain:
                    choosen_feature_num = i
                    best_gain, best_question, best_true_rows, best_true_labels, best_false_rows, best_false_labels = \
                        gain, question, true_rows, true_labels, false_rows, false_labels

        self.used_features.add(self.label_names[choosen_feature_num])
    ...
    def build_tree(self, rows, labels):
        ...
        if len(labels) < self.min_for_pruning or labels.count(labels[0]) == len(labels):
    ...
    def predict(self, rows):
        ...
        for i in range(rows.shape[0]):
            curr_predict = self.predict_sample(rows[i, :])
